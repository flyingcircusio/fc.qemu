...
def show_routes_for_vrf(vrfname):
    def func(iface=None):
        args = ["route", "show", "vrf", vrfname]
        if iface:
            args.extend(["dev", iface])
        v4 = iproute2_json(*(["-4"] + args))
        v6 = iproute2_json(*(["-6"] + args))
        return [
            x
            for x in (v4 + v6)
...
..."""
    ...
    # If therea re extra routes for the guest's tap device which are
